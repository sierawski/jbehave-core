...
public class MarkUnmatchedStepsAsPending implements StepCollector {
    ...
    public List<Step> collectLifecycleSteps(List<CandidateSteps> candidateSteps, Lifecycle lifecycle, Meta storyAndScenarioMeta, Stage stage, Scope scope) {
        ...
        if (stage == Stage.BEFORE) {
            ...
            steps.addAll(collectLifecycleAfterSteps(allCandidates, lifecycle, storyAndScenarioMeta, scope,
                    namedParameters, DEFAULT_STEP_MONITOR));
        }
        return steps;
    }
    ...
    @Override
    public Map<Stage, List<Step>> collectLifecycleSteps(List<CandidateSteps> candidateSteps, Lifecycle lifecycle, Meta storyAndScenarioMeta, Scope scope, StepMonitor stepMonitor) {
        List<StepCandidate> allCandidates = stepFinder.collectCandidates(candidateSteps);
        Map<String, String> namedParameters = new HashMap<>();
        ...
        steps.put(Stage.BEFORE, collectLifecycleBeforeSteps(allCandidates, lifecycle, scope, namedParameters,
                stepMonitor));
        steps.put(Stage.AFTER, collectLifecycleAfterSteps(allCandidates, lifecycle, storyAndScenarioMeta, scope,
                namedParameters, stepMonitor));
        return steps;
    }
